# You can run this here: https://repl.it/Fuie/250

"""
Trees are pretty cool data structures. The basic idea with a Tree
is that it is a node that point to multiple different nodes, each 
...
class BinaryTree(object):
  ...
  def size(self):
    ...
    right_size = 0 if self.right == None else self.right.size()
    return 1 + left_size + right_size
    
  # Return the number of elements in the longest branch of the tree.
  def height(self):
    left_height = 0 if self.left == None else self.left.height()
    right_height = 0 if self.right == None else self.right.height()
  ...
  def contains(self, search_val):
    if self.value == search_val:
      ...
      if self.right == None:
        ...
        return self.right.contains(search_val)
...
class Tree(object):
  ...
  def size(self):
    ...
    return 1 + children_sizes
  
  # Return the number of elements in the longest branch of the tree.
  def height(self):
    biggest_child_height = 0
    for child in self.children:
      child_height = child.height()
      if child_height > biggest_child_height:
        biggest_child_height = child_height
    return 1 + biggest_child_height
  ...
  def contains(self, search_val):
    if self.value == search_val: 
      ...
      return any([child.contains(search_val) for child in self.children])
      # # The `any` keyword is a nifty Python-ism. Alternatively, we could use:
      # for child in self.children:
      #   if child.contains(search_val):
      #     return True
      # return False
  ...
  def get_by_value(self, search_val):
    if self.value == search_val:
      return self #WOAHHH, WHY CAN WE DO THAt? Wait, "self" is just a reference to an object -- that's right.
    
    for child in self.children:
      obj_reference = child.get_by_value(search_val)
      if obj_reference != None:
        return obj_reference
    
    return None #We didn't find it! Oh noez.
